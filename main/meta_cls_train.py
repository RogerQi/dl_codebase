...
def meta_train(cfg, backbone_net, criterion, device, optimizer, meta_train_set, epoch):
    backbone_net.train()
    avg_loss = 0
    meta_loader = get_fs_classification_loader(meta_train_set, 100, 5, 5, 16)
...
def main():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.max_epochs + 1):
        start_cp = time.time()
        meta_train(cfg, backbone_net, criterion, device, optimizer, meta_train_set, epoch)
        scheduler.step()
