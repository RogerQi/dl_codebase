...
def meta_test_one_proto(cfg, backbone_net, criterion, feature_shape, device, meta_test_batch):
    ...
    with torch.no_grad():
        ...
        supp_proto = supp_feature.view((5, 5, 512)).mean(dim=1)
...
def meta_test(cfg, backbone_net, feature_shape, criterion, device, meta_test_set):
    ...
    random.seed(1221)
    acc_list = []
    meta_loader = get_fs_classification_loader(meta_test_set, 600, 5, num_shots, 15)
    for j, meta_test_batch in tqdm(enumerate(meta_loader)):
        meta_test_batch = meta_test_batch[0]

        if 'protonet' in cfg.name:
            batch_acc = meta_test_one_proto(cfg, backbone_net, criterion, feature_shape, device, meta_test_batch)
        else:
            batch_acc = meta_test_one(cfg, backbone_net, criterion, feature_shape, device, meta_test_batch)
