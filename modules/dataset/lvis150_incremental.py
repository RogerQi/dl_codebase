...
from .lvis150 import LVIS150Reader

class LVIS150IncrementalReader(datasets.vision.VisionDataset):
    def __init__(self, root, split, exclude_novel=False, vanilla_label=False):
        ...
        """
        super(LVIS150IncrementalReader, self).__init__(root, None, None, None)
        if vanilla_label:
            assert exclude_novel
        self.vanilla_label = vanilla_label

        # Get augmented VOC dataset
        self.vanilla_ds = LVIS150Reader(root, split)
...
def get_train_set(cfg):
    ...
    ds = LVIS150IncrementalReader(COCO_PATH, True, exclude_novel=False)
    return base_set(ds, "train", cfg)

def get_val_set(cfg):
    COCO_PATH = os.path.join(utils.get_dataset_root(), "COCO2017")
    ds = LVIS150IncrementalReader(COCO_PATH, False, exclude_novel=False)
    return base_set(ds, "test", cfg)

def get_train_set_vanilla_label(cfg):
    COCO_PATH = os.path.join(utils.get_dataset_root(), "COCO2017")
    ds = LVIS150IncrementalReader(COCO_PATH, True, exclude_novel=True, vanilla_label=True)
    return base_set(ds, "train", cfg)
