...
def meta_test_one(cfg, backbone_net, criterion, feature_shape, device, meta_test_batch):
    ...
    with torch.no_grad():
        ...
        fg_vec = utils.masked_average_pooling(supp_mask_bhw_tensor == 1, support_feature, True)  # 1 x C
        bg_vec = utils.masked_average_pooling(supp_mask_bhw_tensor == 0, support_feature, True)  # 1 x C
