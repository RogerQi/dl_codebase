...
from .non_fs_sequential_GIFS_seg_trainer import non_fs_sequential_GIFS_seg_trainer
...
class non_fs_incremental_trainer(non_fs_sequential_GIFS_seg_trainer):
    def __init__(self, cfg, backbone_net, post_processor, criterion, dataset_module, device):
        super(non_fs_incremental_trainer, self).__init__(cfg, backbone_net, post_processor, criterion,
        ...
        if baseset_type == 'random':
            ...
            if not os.path.exists(f"{baseset_folder}/similarity_dic"):
                ...
                for c in base_id_list:
                    mean_weight_dic[c] = \
                    self.prv_post_processor.pixel_classifier.class_mat.weight.data[c]
                ...
                for i in tqdm(range(len(self.train_set))):
                    ...
                    for c in class_list:
                        ...
                        with torch.no_grad():
                            ...
                            img_weight = utils.masked_average_pooling(mask_tensor == c, img_feature,
                                                                      True)
            ...
            for c in base_id_list:
                ...
                if baseset_type == 'uniform_interval':
                    assert m_interval <= len(
                        similarity_dic[c]), f"Need {m_interval} from {len(similarity_dic[c])}"
    ...
    def test_one(self, device, num_runs=5):
        ...
        non_fs_sequential_GIFS_seg_trainer.test_one(self, device, num_runs)
    ...
    def synthesizer_sample(self, novel_obj_id):
        ...
        #     assert len(self.partial_data_pool[
        #                    k]) == num_novel_instances, "every class is expected to have $numShot$ samples"
        if self.cfg.TASK_SPECIFIC.GIFS.probabilistic_synthesis_strat == 'vRFS':
            ...
            raise NotImplementedError("Unknown probabilistic synthesis strategy: {}".format(
                self.cfg.TASK_SPECIFIC.GIFS.probabilistic_synthesis_strat))
    ...
    def get_scene_embedding(self, img):
        ...
        with torch.no_grad():
            ...
            norm = torch.norm(scene_embedding, p=2, dim=1).unsqueeze(1).expand_as(
                scene_embedding)  # norm
    ...
    def finetune_backbone(self, base_class_idx, novel_class_idx, support_set):
        ...
        if self.context_aware_prob > 0:
            for novel_obj_id in novel_class_idx:
                ...
                for idx in support_set[novel_obj_id]:
                    ...
                    similarity_score = F.cosine_similarity(scene_embedding,
                                                           self.base_pool_cos_embeddings)
                    base_candidates = torch.argsort(similarity_score)[-int(
                        0.1 * self.base_pool_cos_embeddings.shape[0]):]  # Indices array
                    if novel_obj_id not in self.context_similar_map:
                        self.context_similar_map[novel_obj_id] = list(base_candidates.cpu().numpy())
                    else:
                        self.context_similar_map[novel_obj_id] += list(
                            base_candidates.cpu().numpy())
        ...
        trainable_params = [
            ...
            {"params": self.post_processor.parameters(),
             "lr": self.cfg.TASK_SPECIFIC.GIFS.classifier_lr}
        ]
        ...
        optimizer = optim.SGD(trainable_params, lr=self.cfg.TASK_SPECIFIC.GIFS.backbone_lr,
                              momentum=0.9)
