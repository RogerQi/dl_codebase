...
class fs_incremental_trainer(sequential_GIFS_seg_trainer):
    ...
    def synthesizer_sample(self, novel_obj_id):
        ...
        if torch.rand(1) < 0.5:
            ...
            # For 50% of time, sample an image from blank memory bank
            syn_img_chw, syn_mask_hw = random.choice(self.blank_bank[novel_obj_id])
            num_novel_objects = 1
    ...
    def finetune_backbone(self, base_class_idx, novel_class_idx, supp_img_bchw, supp_mask_bhw):
        ...
        for b in range(supp_img_bchw.shape[0]):
            ...
            for ctr in contours:
                ...
                base_img_chw = torch.zeros((3, 512, 512))
                base_mask_hw = torch.zeros((512, 512))
                # all blank background with object only
                img_chw, mask_hw = self.copy_and_paste(img_roi, mask_roi, base_img_chw, base_mask_hw, novel_obj_id)
                if novel_obj_id not in self.blank_bank:
                    self.blank_bank[novel_obj_id] = []
                self.blank_bank[novel_obj_id].append((img_chw, mask_hw))
